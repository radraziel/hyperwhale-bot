...
MAX_MSGS_PER_CYCLE = 5           # si hay más, se envía un resumen
...
def _sleep_until_next_slot():
    """Respeta el throttle de envío por chat."""
...
def send_telegram(text: str, max_retries: int = 3):
    """
    Envía mensaje a Telegram respetando throttle y reintenta si hay 429.
    """
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        print("❌ Falta TELEGRAM_TOKEN o TELEGRAM_CHAT_ID en variables de entorno.")
        return
        ...
        try:
            ...
            if r.status_code == 429:
                ...
                try:
                    ...
                    retry_after = 3
                print(f"⏳ Rate limit (429). Esperando {retry_after}s y reintentando…")
                time.sleep(int(retry_after) + 1)
                attempt += 1
                continue

            if not r.ok:
                print("⚠️ Error al enviar a Telegram:", r.status_code, r.text[:200])
...
def fetch_fills_resilient(address: str, last_ts: int):
    """
    Intenta múltiples variantes conocidas para obtener fills por usuario.
    Devuelve lista de fills normalizados (posiblemente vacía).
    """
    trials = [
    ...
    if errors:
        print("❌ No pude obtener fills. Detalle:")
        for e in errors:
            print("   -", e)
    return []
...
def build_message(addr: str, f: dict):
    parts = []
    parts.append("⚡ Actividad detectada")
    ...
    parts.append(f"Tamaño: {f.get('sz', '?')}")
...
def run_bot():
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID or not HL_TRADER_ADDRESS:
        print("❌ Faltan variables: TELEGRAM_TOKEN / TELEGRAM_CHAT_ID / HL_TRADER_ADDRESS.")
        return

    print("✅ Iniciando bot HyperWhaleBot (Replit / rate-safe)…")
